"""
Description: Probabilistic Roadmap algorithm
"""
...
def AStar(self, robot, start, goal):
    q = []
    global_v.DISTANCE[robot.position[0]][robot.position[1]] = 0
    values = robot.position[0], robot.position[1], robot.heading
    heapq.heappush(q, (global_v.DISTANCE[robot.position[0]][robot.position[1]] + calc_dist(start, goal), values))
    global_v.PREVIOUS[position[0]][position[1]] = None
    delta_t = 0.5
...
def trace_path(self, robot, start, end):
    ...
    while (end != start):
        result.append(end)
        end = global_v.PREVIOUS[end[0]][end[1]]
    result.append(start)
    result.reverse()
    for i in result:
...
def PRM(world, robot, start, goal):
    ...
    q = []
    global_v.DISTANCE[start_position[0]][start_position[1]] = 0
    dist = calc_dist(start_position, goal_position)
    heapq.heappush(q, (global_v.DISTANCE[start_position[0]][start_position[1]] + dist, len(world.VERTICES) - 2))
    while (len(q) != 0):
        priority, u = heapq.heappop(q)
        ...
        if (u != len(world.VERTICES) - 1):
            for v in world.EDGES[u]:
                ...
                if (global_v.DISTANCE[node_v[0]][node_v[1]] > global_v.DISTANCE[node_u[0]][node_u[1]] + calc_dist(node_u, node_v)):
                    dist = calc_dist(node_u, node_v)
                    global_v.DISTANCE[node_v[0]][node_v[1]] = global_v.DISTANCE[node_u[0]][node_u[1]] + dist
                    heapq.heappush(q, (global_v.DISTANCE[node_v[0]][node_v[1]] + calc_dist(node_v, goal_position), v))
